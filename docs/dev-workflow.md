...
現在は `frontend/package.json` の scripts に合わせ、次を実行する。

- **Dev**: `pnpm --filter frontend run dev`
- **Build**: `pnpm --filter frontend run build`
- **Lint**: `pnpm --filter frontend run lint`
- **Typecheck**: `pnpm --filter frontend exec tsc --noEmit`（必要時。将来的に `typecheck` scriptへ統合）
...
- **frontend-check**
  ...
  - `pnpm --filter frontend run build`（`tsc -b` を含む）
  - （導入後）`pnpm --filter frontend exec prettier --check .`
  - （導入後）ユニットテスト（Vitest等）
- **security**
